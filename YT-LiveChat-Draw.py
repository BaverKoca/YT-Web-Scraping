...
browser_data = webdriver.SearchEngine() # You can use any kind of search engine you want. Just check it is give permission to scraping.
eligibleUsers = set()

def getHTML(url, browser_instance):
    browser_instance.get(url)
    time.sleep(5) # You  can change time for wait there. But if chat is so crowded or your internet speed is slow, you can change till 30 second.
...
browser_data.quit() # Quit from search engine.
time.sleep(3) # You  can change time for wait there.

# This for loop is optional where's upder here. You don't have to use that loop if you wanna see the winner quick. You can delete or put in comment line until here to...
for i in range(1, 4):
    dots = i * "."
